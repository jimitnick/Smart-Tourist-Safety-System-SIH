  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "login") {
      ...
      if (email && password) {
        ...
        router.replace("/authority/signup");
      } else {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50">
      <div className="w-full max-w-md rounded-lg shadow-lg bg-white p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            className="w-full py-2 px-4 bg-[#008994] text-white font-semibold rounded-md hover:bg-[#064d52] transition pointer cursor-pointer"
          >
        ...
        <p className="mt-4 text-center text-sm text-gray-600">
          {(
            <>
              ...
              <button
                onClick={() => router.replace("/authority/signup")}
